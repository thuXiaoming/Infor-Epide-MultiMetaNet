function [p_US_final,p_AS_final,p_UE_final,p_AE_final,p_AI_final,p_AR_final]=emu_para(lambda1_u,mu1,phi_0)
...
p_US(:,1)=0.999;p_AS(:,1)=0;p_UE(:,1)=0;
p_AE(:,1)=0;p_AI(:,1)=0.001;p_AR(:,1)=0;
n_pop(:,1)=500;epsilon_0=0.3;

ksi=0.5;   %spread parameters of pyhsical layers 
lambda2=0.05;mu2=0.15;           %spread parameters of virtual layers
%lambda2=0;mu2=0;

kappa=0.5;alpha=0.3;
lambda1_a=kappa*lambda1_u;
phi=phi_0*ones(N,1);  %mobility rate of each patch
...
p_US_final=p_US_ave(end);p_AS_final=p_AS_ave(end);p_UE_final=p_UE_ave(end);
p_AE_final=p_AE_ave(end);p_AI_final=p_AI_ave(end);p_AR_final=p_AR_ave(end);
end